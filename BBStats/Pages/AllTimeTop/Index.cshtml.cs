...
namespace BBStats.Pages.AllTimeTop;
...
public class IndexModel : PageModel
{
	...
	public async Task<IActionResult> OnGetAsync(CancellationToken cancellationToken)
	{
		...
		if (pageNumber < 1)
		{
			return Redirect(PageQueryUrl.WithQueryPage(Url, "/AllTimeTop/Index", 1)!);
		}

		Top = await _topPlayersService.GetPageAsync(pageNumber, isAllTime: true, cancellationToken: cancellationToken);

		if (pageNumber > Top.TotalPages && Top.TotalPages > 0)
		{
			return Redirect(PageQueryUrl.WithQueryPage(Url, "/AllTimeTop/Index", Top.TotalPages)!);
		}
	...
	public string? GetPageUrl(int page) =>
		PageQueryUrl.WithQueryPage(Url, "/AllTimeTop/Index", page);
}
